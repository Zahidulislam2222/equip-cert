...
const statusConfig = {
  ...
  retired: { label: 'Retired', color: 'bg-muted text-muted-foreground dark:bg-muted dark:text-muted-foreground', icon: Wrench },
};
...
export function EquipmentCard({ equipment, onClick }: { equipment: Equipment; onClick?: () => void }) {
  ...
  return (
    <MotionCard className="h-full">
      <button
        onClick={onClick}
        className="w-full h-full text-left rounded-lg border border-border bg-card p-5 shadow-sm hover:shadow-card transition-shadow duration-300 group"
      >
