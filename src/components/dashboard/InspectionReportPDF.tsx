...
import { buildReportProvenance } from '@/lib/pdf-provenance';
...
const styles = StyleSheet.create({
  ...
  // Darker than the footer on purpose: a disclosure a reader cannot make out is not a disclosure.
  disclosure: { position: 'absolute', bottom: 44, left: 30, right: 30, textAlign: 'center', fontSize: 8, color: '#444' }
});
...
export const InspectionReportPDF = ({ data }: { data: InspectionData }) => {
  // Art. 50 marking: machine-readable in the Info dictionary, human-readable in the footer.
  const provenance = buildReportProvenance(data, config.app.name);
  return (
  <Document {...provenance.metadata} language="en">
    <Page size="A4" style={styles.page}>
      ...
      {/* Footer. The AI disclosure sits directly above it, on every AI-assisted report. */}
      {provenance.disclosure && (
        <Text style={styles.disclosure}>{provenance.disclosure}</Text>
      )}
      <Text style={styles.footer}>
        Generated by {config.app.name} • Authorized by {data.inspector_name} • Securely Stored in Supabase
      </Text>
